...
export class SimplyHiredScraper extends BaseScraper {
  ...
  async scrape(params: ScraperParams): Promise<Job[]> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`SimplyHired returned ${response.status}`);
        return jobs;
      }
      ...
      if (!match) {
        console.warn('SimplyHired: no __NEXT_DATA__ found');
        return jobs;
      }
      ...
      if (rawJobs.length === 0) {
        console.warn('SimplyHired: no jobs in page props');
        return jobs;
      }
      ...
      console.log(`SimplyHired: ${jobs.length} jobs from search`);
