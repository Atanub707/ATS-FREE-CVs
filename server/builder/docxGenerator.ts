...
export function generatePdfBuffer(cv: TailoredCv, template: string = 'harvard'): Promise<Buffer> {
  ...
  if (template === 'modern-split') {
    return generateModernSplitPdf(cv);
  }
...
/**
 * Modern Split — navy sidebar (contact, skills, education, certifications)
 * + white main column (summary, experience, projects). Two independent
 * columns flow with per-column page breaks; the navy sidebar is redrawn
 * on every page. US Letter, full-bleed sidebar.
 */
function generateModernSplitPdf(cv: TailoredCv): Promise<Buffer> {
  const PAGE_W = 612;
  const PAGE_H = 792;
  const NAVY = '#0F172A';
  const TEAL = '#2DD4BF';
  const SIDE_TEXT = '#CBD5E1';
  const SIDE_MUTED = '#94A3B8';
  const SIDE_FAINT = '#64748B';
  const SIDE_BOLD = '#E2E8F0';
  const BODY = '#1F2937';
  const MUTED = '#6B7280';
  const FAINT = '#9CA3AF';

  const SIDE_W = 2.35 * 72;          // 169.2 pt
  const SIDE_PAD_X = 20;
  const SIDE_PAD_Y = 24;
  const SIDE_TOP = SIDE_PAD_Y;
  const SIDE_TEXT_W = SIDE_W - SIDE_PAD_X * 2; // 129.2

  const MAIN_X = SIDE_W + 27.4;      // 196.6
  const MAIN_TOP = 26;
  const MAIN_W = PAGE_W - MAIN_X - 27.4; // 388
  const pageBottom = PAGE_H - 32.4;
  ...
  return new Promise((resolve, reject) => {
    ...
    const paintSidebar = () => {
      doc.save();
      doc.rect(0, 0, SIDE_W, PAGE_H).fill(NAVY);
      doc.restore();
    };
    paintSidebar();
    ...
    const sideText = (text: string, x: number, y: number, opts: { font?: string; size?: number; color?: string; bold?: boolean }) => {
      doc.font(opts.bold ? 'Helvetica-Bold' : 'Helvetica').fontSize(opts.size || 8).fillColor(opts.color || SIDE_TEXT);
      doc.text(text, x, y, { width: SIDE_TEXT_W, lineGap: 1 });
      const h = doc.heightOfString(text, { width: SIDE_TEXT_W });
      doc.x = 0;
      return h;
    };
    const mainText = (text: string, x: number, y: number, opts: { font?: string; size?: number; color?: string; bold?: boolean; width?: number }) => {
      const w = opts.width || MAIN_W;
      doc.font(opts.bold ? 'Helvetica-Bold' : 'Helvetica').fontSize(opts.size || 9.5).fillColor(opts.color || BODY);
      doc.text(text, x, y, { width: w, lineGap: 1.2 });
      const h = doc.heightOfString(text, { width: w });
      doc.x = MAIN_X;
      return h;
    };

    // ── Sidebar: name, role, contact ──
    const name = sanitizeText(cv.candidateName).toUpperCase() || 'CANDIDATE NAME';
    ensureSide(120);
    {
      const y0 = yS;
      ...
      for (const ch of Array.from(name)) {
        doc.fillColor('#FFFFFF').text(ch, cx, y0, { lineBreak: false });
        cx += doc.widthOfString(ch) + 2;
      }
      yS = y0 + 21;
    }
    if (cv.targetRole) {
      ...
      if (role) {
        ensureSide(14);
        doc.font('Helvetica-Bold').fontSize(8.5).fillColor(TEAL).text(role, SIDE_PAD_X, yS, { width: SIDE_TEXT_W });
        yS += doc.heightOfString(role, { width: SIDE_TEXT_W }) + 8;
      }
    }
    const contactLinks = getContactLinks(cv);
    const contactLines = contactLinks.map((c) => ({ label: sanitizeText(c.label), url: c.url ? normalizeUrl(c.url) : undefined }));
    for (const line of contactLines) {
      ensureSide(26);
      const labelH = 9;
      const y0 = yS;
      doc.font('Helvetica-Bold').fontSize(7).fillColor(SIDE_FAINT)
        .text(line.url ? 'LINK' : 'DETAIL', SIDE_PAD_X, y0, { width: SIDE_TEXT_W });
      if (line.url) {
        doc.font('Helvetica').fontSize(7.5).fillColor(TEAL).text(line.label, SIDE_PAD_X, y0 + labelH, { width: SIDE_TEXT_W });
        const h = doc.heightOfString(line.label, { width: SIDE_TEXT_W });
        doc.link(SIDE_PAD_X, y0 + labelH, SIDE_TEXT_W, h, line.url);
        yS = y0 + labelH + h + 2;
      } else {
        doc.font('Helvetica').fontSize(7.5).fillColor(SIDE_MUTED).text(line.label, SIDE_PAD_X, y0 + labelH, { width: SIDE_TEXT_W });
        const h = doc.heightOfString(line.label, { width: SIDE_TEXT_W });
        yS = y0 + labelH + h + 3;
      }
      doc.x = 0;
    ...
    const sideHeader = (title: string) => {
      ensureSide(24);
      const y0 = yS;
      doc.font('Helvetica-Bold').fontSize(9).fillColor(SIDE_BOLD)
        .text(title.toUpperCase(), SIDE_PAD_X, y0, { width: SIDE_TEXT_W, characterSpacing: 0 });
      doc.moveTo(SIDE_PAD_X, y0 + 11).lineTo(SIDE_W - SIDE_PAD_X, y0 + 11)
        .lineWidth(1).strokeColor('rgba(255,255,255,0.22)').stroke();
      yS = y0 + 13;
      doc.x = 0;
    };
    ...
    if (skillCats.length > 0 || useCompetencies) {
      sideHeader('Skills');
      if (useCompetencies) {
        const comp = cv.coreCompetencies.map((c) => sanitizeText(c)).filter(Boolean).join(', ');
        ensureSide(20);
        doc.font('Helvetica').fontSize(8).fillColor(SIDE_MUTED).text(comp, SIDE_PAD_X, yS, { width: SIDE_TEXT_W, lineGap: 1 });
        yS += doc.heightOfString(comp, { width: SIDE_TEXT_W }) + 3;
        doc.x = 0;
      } else {
        for (const cat of skillCats) {
          const line1 = cat.name + ':';
          const line2 = cat.list;
          const h1 = doc.heightOfString(line1, { width: SIDE_TEXT_W });
          const h2 = doc.heightOfString(line2, { width: SIDE_TEXT_W });
          ensureSide(h1 + h2 + 6);
          doc.font('Helvetica-Bold').fontSize(7.5).fillColor(SIDE_BOLD).text(line1, SIDE_PAD_X, yS, { width: SIDE_TEXT_W });
          doc.font('Helvetica').fontSize(7.5).fillColor(SIDE_MUTED).text(line2, SIDE_PAD_X, yS + h1 + 1, { width: SIDE_TEXT_W });
          yS += h1 + h2 + 3;
          doc.x = 0;
        }
      }
    ...
    if (cv.education && cv.education.length > 0) {
      ...
      for (const edu of cv.education) {
        ...
        const degree = sanitizeText(edu.degree);
        const eDates = sanitizeText(edu.dates);
        const h1 = doc.heightOfString(inst, { width: SIDE_TEXT_W });
        const h2 = doc.heightOfString([degree, eDates].filter(Boolean).join(' | '), { width: SIDE_TEXT_W });
        ensureSide(h1 + h2 + 6);
        doc.font('Helvetica-Bold').fontSize(7.5).fillColor(SIDE_BOLD).text(inst, SIDE_PAD_X, yS, { width: SIDE_TEXT_W });
        doc.font('Helvetica').fontSize(7.5).fillColor(SIDE_MUTED)
          .text([degree, eDates].filter(Boolean).join(' | '), SIDE_PAD_X, yS + h1 + 1, { width: SIDE_TEXT_W });
        yS += h1 + h2 + 5;
        doc.x = 0;
      }
    }
    ...
    if (cv.certifications && cv.certifications.length > 0) {
      ...
      for (const cert of cv.certifications) {
        ...
        if (typeof cert === 'string') nameT = sanitizeText(cert);
          ...
          nameT = sanitizeText(cert.name);
          issuer = sanitizeText(cert.issuer);
        }
        if (!nameT) continue;
        const line1 = issuer;
        const line2 = nameT;
        const h1 = line1 ? doc.heightOfString(line1, { width: SIDE_TEXT_W }) : 0;
        const h2 = doc.heightOfString(line2, { width: SIDE_TEXT_W });
        ensureSide(h1 + h2 + 6);
        if (line1) {
          doc.font('Helvetica-Bold').fontSize(7.5).fillColor(SIDE_BOLD).text(line1, SIDE_PAD_X, yS, { width: SIDE_TEXT_W });
        }
        doc.font('Helvetica').fontSize(8.5).fillColor(SIDE_MUTED).text(line2, SIDE_PAD_X, yS + h1 + 1, { width: SIDE_TEXT_W });
        yS += h1 + h2 + 5;
        doc.x = 0;
      }
    }
    ...
    const mainHeader = (title: string) => {
      ...
      doc.font('Helvetica-Bold').fontSize(10.5).fillColor(NAVY)
        .text(title.toUpperCase(), MAIN_X, y0, { width: MAIN_W });
      doc.moveTo(MAIN_X, y0 + 12).lineTo(PAGE_W - 27.4, y0 + 12).lineWidth(1.2).strokeColor('#E2E8F0').stroke();
      yM = y0 + 14;
      doc.x = MAIN_X;
    };
    const mainBullet = (text: string) => {
      if (!text) return;
      const clean = sanitizeText(String(text).replace(/^[*•\-]\s*/, '').trim());
      if (!clean) return;
      const bulletX = MAIN_X + 2;
      const textX = MAIN_X + 14;
      const tWidth = MAIN_W - 14;
      const h = doc.heightOfString(clean, { width: tWidth });
      ensureMain(h + 3);
      const y0 = yM;
      doc.font('Helvetica').fontSize(9.5).fillColor(TEAL).text('\u2022', bulletX, y0, { lineBreak: false });
      doc.font('Helvetica').fontSize(9.5).fillColor(BODY).text(clean, textX, y0, { width: tWidth, lineGap: 0.9 });
      yM = y0 + h + 2.5;
      doc.x = MAIN_X;
    };
    ...
    if (cv.professionalSummary) {
      ...
      if (cleanSummary) {
        mainHeader('Summary');
        const h = doc.heightOfString(cleanSummary, { width: MAIN_W });
        ensureMain(h + 4);
        doc.font('Helvetica').fontSize(9.5).fillColor(BODY).text(cleanSummary, MAIN_X, yM, { width: MAIN_W, lineGap: 1 });
        yM += h + 4;
        doc.x = MAIN_X;
      }
    }
    ...
    if (cv.workExperience && cv.workExperience.length > 0) {
      ...
      for (const exp of cv.workExperience) {
        ...
        const headH = 14 + (loc ? 12 : 0);
        ensureMain(headH);
        const y0 = yM;
        doc.font('Helvetica-Bold').fontSize(10.5).fillColor(NAVY).text(title, MAIN_X, y0, { continued: true });
        doc.fillColor(TEAL).text(company ? '  \u2014  ' + company : '', { continued: true });
        doc.x = MAIN_X;
        if (period) {
          doc.font('Helvetica').fontSize(9).fillColor(MUTED).text(period, MAIN_X, y0, { align: 'right', width: MAIN_W });
        }
        yM = y0 + 9;
        if (loc) {
          ensureMain(12);
          doc.font('Helvetica').fontSize(9).fillColor(MUTED).text(loc, MAIN_X, yM, { width: MAIN_W });
          yM += 11;
        }
        doc.x = MAIN_X;
        if (Array.isArray(exp.highlights)) {
          for (const hl of exp.highlights) mainBullet(hl);
        }
        yM += 2;
        doc.x = MAIN_X;
      }
    ...
    if (cv.projects && cv.projects.length > 0) {
      ...
      for (const proj of cv.projects) {
        ...
        const normLink = proj.link ? normalizeUrl(proj.link) : undefined;
        const desc = sanitizeText(proj.description);
        const hDesc = desc ? doc.heightOfString(desc, { width: MAIN_W }) : 0;
        const hLink = normLink ? 12 : 0;
        ensureMain(14 + hDesc + hLink + 6);

        const y0 = yM;
        doc.font('Helvetica-Bold').fontSize(9.5).fillColor(NAVY).text(pName, MAIN_X, y0, { continued: true });
        if (pDates) {
          doc.font('Helvetica').fontSize(9).fillColor(MUTED).text('  [' + pDates + ']');
        }
        yM = y0 + 10;
        doc.x = MAIN_X;
        if (desc) {
          doc.font('Helvetica').fontSize(9.5).fillColor(BODY).text(desc, MAIN_X, yM, { width: MAIN_W, lineGap: 1 });
          yM += hDesc + 1;
          doc.x = MAIN_X;
        }
        if (normLink) {
          ensureMain(12);
          doc.font('Helvetica').fontSize(9.5).fillColor(TEAL).text(normLink, MAIN_X, yM, { width: MAIN_W });
          yM += hLink;
          doc.x = MAIN_X;
        }
        yM += 2;
