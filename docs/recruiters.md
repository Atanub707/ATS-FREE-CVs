...
## AI Interview (job-description grounded)

Navbar **AI Interview** opens the mock-interview screen. A senior interviewer that reads the jobs actually scraped in the dashboard:
1. **Intro (3 questions)** — role from the dashboard (real role chips with job counts from `GET /api/interview/roles`), years of experience, optional pick a real posting from your list (`GET /api/interview/jobs?role=…`).
2. **Q&A (7 questions)** — every question is grounded in a real JD: the prompt includes the posting's title/company + description excerpt, so questions probe the actual tools and scenarios in that JD. Answers are scored 0–10 with one-line feedback.
3. **Rubric scoring** — each answer is scored on 4 dimensions (ACCURACY 40%, DEPTH 25%, STRUCTURE 15%, EXAMPLES 20%); the overall is computed server-side, short answers are capped (≤4 for one-liners, ≤6 under 25 words), and the UI shows the dimension breakdown.
4. **Scorecard** — overall ring, per-question rows, verdict grounded in the weakest answer.

- `POST /api/interview/start` `{ role, experienceYears?, jobId? }` → `{ sessionId, question, jobTitle, company, questionIndex, total }`
- `POST /api/interview/answer` `{ sessionId, answer }` → `{ done:false, score, feedback, dims, question, jobTitle, company }` or `{ done:true, scorecard }`
...
## AI System (Interview + Personalize)

Navbar **AI System** opens a landing with two 50/50 assistants:

### Interview with AI (job-description grounded)
A senior interviewer that reads the jobs actually scraped in the dashboard:
1. **Intro (3 questions)** — role from the dashboard (real role chips with job counts from `GET /api/interview/roles`), years of experience, optional pick a real posting from your list (`GET /api/interview/jobs?role=…`).
2. **Q&A (7 questions)** — every question is grounded in a real JD: the prompt includes the posting's title/company + description excerpt, so questions probe the actual tools and scenarios in that JD. Answers are scored 0–10 with one-line feedback.
3. **Scorecard** — overall ring, per-question rows, verdict.

- `POST /api/interview/start` `{ role, experienceYears?, jobId? }` → `{ sessionId, question, jobTitle, company, questionIndex, total }`
- `POST /api/interview/answer` `{ sessionId, answer }` → `{ done:false, score, feedback, question, jobTitle, company }` or `{ done:true, scorecard }`
- **Where:** `server/interview.ts` (role aggregation, JD question bank, session), routes in `server.ts`, UI in `src/components/AiSystemScreen.tsx`.
- **Tested:** `tests/recruiters/interview.test.ts` (role aggregation, JD-grounded question, scoring, scorecard).

### Personalized AI Assistant
Coming soon — analyzes stored missing-keyword data across scored jobs and updates the Master CV on approval.

> The generic job-search chat (MCP tool chat) was removed from the UI (2026-08-15). `/api/chat` + the MCP tool loop remain server-side for future use.
