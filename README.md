...
5. Check the **history** (Clock button) to see every past email with subject, body, attachment, and timestamp — or save reusable **templates** from the compose modal
6. **Batch mode**: tick contacts (or "Select all visible") and **Send N** — emails go out one by one with a progress chip, and closing the modal cancels the queue; **Dismiss (N)** hides the selected contacts
7. **Verify** an email's format + domain MX, message WhatsApp contacts via the **WhatsApp** link, and export the whole list as **CSV**
